...
let handler = async (m, { conn }) => {
  ...
  if (!/^❓ Quanto e o resultado de/i.test(m.quoted.text)) return
  if (!(m.chat in global.math)) return conn.reply(m.chat, '*Esa pregunta ja acabou*', m)
  ...
  if (m.text == math.result) {
    ...
    if (--global.math[id][2] == 0) {
      conn.reply(m.chat, `🙀 *Se acabo seu tempo!*\nRespuesta: *${math.result}*`, m)
      clearTimeout(global.math[id][3])
      delete global.math[id]
